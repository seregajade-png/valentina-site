  ...
  return (
    <>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className="grid h-11 w-11 place-items-center rounded-full border border-line-2 text-ink-dim transition hover:border-accent hover:text-accent md:hidden"
      >
      ...
      <AnimatePresence>
        ...
        {open && (
          <motion.aside
            key="drawer"
            className="fixed right-0 top-0 z-50 flex h-full w-[88%] max-w-[400px] flex-col border-l border-line bg-bg p-8 pt-24 md:hidden"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
